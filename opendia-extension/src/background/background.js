  ...
  async createConnection() {
    try {
      ...
      this.mcpSocket.onmessage = async (event) => {
        ...
        // Phase 4 chat_* frames route through the chat store, not the
        // browser tool dispatcher — they're bridge-only per SPEC §5.1.
        if (typeof globalThis.__opendiaHandleChatFrame === 'function') {
          const handled = await globalThis.__opendiaHandleChatFrame(message);
          if (handled) return;
        }
...
browser.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener(async (frame) => {
    try {
      ...
      // Chat frames intercept ahead of the browser dispatcher.
      if (typeof globalThis.__opendiaHandleChatFrame === "function") {
        const handled = await globalThis.__opendiaHandleChatFrame(
...
// Phase 4 chat store. Extension owns durable chat state; daemon proxies
// reads/writes via chat_* frames on the existing WebSocket. Storage layout:
//   cebian:chats:index      -> { chat_ids: string[] }
//   cebian:chats:<chat_id>  -> ChatSession
//
// Wire protocol (SPEC §5.1): {id, method, params} in, {id, result|error} out.
// Server-push: {type:'chat_appended', sub_id, chat_id, msg} — sent over the
// same WS to the daemon; subscribers on the daemon side match by sub_id.
const CHAT_INDEX_KEY = "cebian:chats:index";
const CHAT_KEY_PREFIX = "cebian:chats:";
const chatSubscribers = new Map(); // sub_id -> { chat_id, last_msg_id }
const chatRuntimeSubscribers = new Map(); // runtime port sub_id → port for sidepanel push
const chatIdRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;

function uuidv4() {
  if (typeof crypto !== "undefined" && crypto.randomUUID) return crypto.randomUUID();
  const b = new Uint8Array(16);
  (crypto?.getRandomValues || ((x) => x.fill(0)))(b);
  b[6] = (b[6] & 0x0f) | 0x40;
  b[8] = (b[8] & 0x3f) | 0x80;
...
function storageSet(pairs) {
  return new Promise((resolve) => {
    try { browser.storage.local.set(pairs, () => resolve()); }
    catch { resolve(); }
  });
}
...
async function chatLoad(chatId) {
  const key = CHAT_KEY_PREFIX + chatId;
  const r = await storageGet([key]);
  return r[key] || null;
}

async function chatSave(chat) {
  const key = CHAT_KEY_PREFIX + chat.chat_id;
  await storageSet({ [key]: chat });
}

async function chatIndexList() {
  const r = await storageGet([CHAT_INDEX_KEY]);
  return (r[CHAT_INDEX_KEY] && Array.isArray(r[CHAT_INDEX_KEY].chat_ids))
    ? r[CHAT_INDEX_KEY].chat_ids
    : [];
}

async function chatIndexAdd(chatId) {
  const ids = await chatIndexList();
  if (!ids.includes(chatId)) {
    ids.push(chatId);
    await storageSet({ [CHAT_INDEX_KEY]: { chat_ids: ids } });
  }
}

async function chatIndexRemove(chatId) {
  const ids = await chatIndexList();
  const next = ids.filter((x) => x !== chatId);
  await storageSet({ [CHAT_INDEX_KEY]: { chat_ids: next } });
}

function broadcastChatAppended(chatId, msg) {
  // Wake daemon-side subscribers via the WS (server-push, no id).
  for (const [subId, sub] of chatSubscribers) {
    if (sub.chat_id !== chatId) continue;
    sub.last_msg_id = msg.msg_id;
  ...
  // Wake in-extension subscribers (Phase 3 sidepanel chat surface).
  for (const [subId, port] of chatRuntimeSubscribers) {
    try { port.postMessage({ type: "chat_appended", sub_id: subId, chat_id: chatId, msg }); }
    catch { /* port closed; runtime.onDisconnect cleans up */ }
  }
}

async function chatCreate({ title, tab_hint } = {}) {
  const chat = {
    chat_id: uuidv4(),
  ...
  if (tab_hint != null) chat.tab_hint = tab_hint;
  await chatSave(chat);
  await chatIndexAdd(chat.chat_id);
  return { chat_id: chat.chat_id };
}

async function chatSend({ chat_id, client_msg_id, role, text, tool_call, metadata }) {
  if (!chatIdRegex.test(chat_id || "")) throw new Error("CHAT_NOT_FOUND");
  if (!client_msg_id) throw new Error("INVALID_ROLE: client_msg_id required");
  if (!["user", "assistant", "tool"].includes(role)) throw new Error(`INVALID_ROLE: ${role}`);
  const chat = await chatLoad(chat_id);
  if (!chat) throw new Error("CHAT_NOT_FOUND");
  const dup = chat.messages.find((m) => m.client_msg_id === client_msg_id);
  if (dup) {
    if (dup.role !== role || dup.text !== text) throw new Error("IDEMPOTENCY_CONFLICT");
    return { msg_id: dup.msg_id, ts: dup.ts };
  }
  const msg = {
    msg_id: ++chat.msg_counter,
    client_msg_id,
    ts: Date.now(),
    role,
    text,
  ...
  await chatSave(chat);
  broadcastChatAppended(chat_id, msg);
  return { msg_id: msg.msg_id, ts: msg.ts };
}

async function chatRead({ chat_id, since_msg_id, limit }) {
  if (!chatIdRegex.test(chat_id || "")) throw new Error("CHAT_NOT_FOUND");
  const chat = await chatLoad(chat_id);
  if (!chat) throw new Error("CHAT_NOT_FOUND");
  let msgs = chat.messages;
  if (typeof since_msg_id === "number") msgs = msgs.filter((m) => m.msg_id > since_msg_id);
  if (typeof limit === "number" && limit > 0) msgs = msgs.slice(0, limit);
  return { chat_id, messages: msgs };
}
...
async function chatList() {
  ...
  for (const chat_id of ids) {
    const c = await chatLoad(chat_id);
    if (!c) continue;
    chats.push({
      chat_id,
      title: c.title,
...
async function chatDelete({ chat_id }) {
  if (!chatIdRegex.test(chat_id || "")) throw new Error("CHAT_NOT_FOUND");
  await storageRemove([CHAT_KEY_PREFIX + chat_id]);
  await chatIndexRemove(chat_id);
  // Notify subscribers still watching this chat.
  for (const [subId, sub] of chatSubscribers) {
    if (sub.chat_id !== chat_id) continue;
    connectionManager.send({ type: "chat_deleted", sub_id: subId, chat_id });
  }
  for (const [subId, port] of chatRuntimeSubscribers) {
    try { port.postMessage({ type: "chat_deleted", sub_id: subId, chat_id }); }
    catch {}
  }
  return { ok: true };
}

function chatSubscribe({ chat_id, sub_id, since_msg_id }) {
  if (!chatIdRegex.test(chat_id || "")) throw new Error("CHAT_NOT_FOUND");
  if (!sub_id) throw new Error("INVALID_ROLE: sub_id required");
  chatSubscribers.set(sub_id, { chat_id, last_msg_id: since_msg_id ?? null });
  return { ok: true, sub_id };
}

function chatUnsubscribe({ sub_id }) {
  chatSubscribers.delete(sub_id);
  return { ok: true };
}

// Wire chat_* frames into the existing WS dispatch. Bypasses handleMCPRequest
// because these frames aren't in the getAvailableTools() surface — they're
// bridge-only messages per SPEC §5.1.
const CHAT_METHODS = {
  chat_list: () => chatList(),
...
async function handleChatFrame(message, replyTo) {
  const method = message?.method;
  if (!method || !CHAT_METHODS[method]) return false;
  const emit = replyTo || ((m) => connectionManager.send(m));
  try {
    const result = await CHAT_METHODS[method](message.params || {});
    emit({ id: message.id, result });
  } catch (err) {
    emit({ id: message.id, error: { message: err?.message ?? String(err), code: -32603 } });
  }
  return true;
}
globalThis.__opendiaHandleChatFrame = handleChatFrame;

// Runtime port for sidepanel chat subscribers — separate name so it doesn't
// clash with the loopback MCP transport.
browser.runtime.onConnect.addListener((port) => {
  if (port.name !== "cebian-chat") return;
  const localSubs = new Set();
  port.onMessage.addListener(async (frame) => {
    if (!frame || typeof frame !== "object") return;
    try {
      if (frame.type === "subscribe") {
        const subId = frame.sub_id || uuidv4();
        chatRuntimeSubscribers.set(subId, port);
        localSubs.add(subId);
        port.postMessage({ type: "subscribed", sub_id: subId, chat_id: frame.chat_id });
        return;
      }
      if (frame.type === "unsubscribe") {
        chatRuntimeSubscribers.delete(frame.sub_id);
        localSubs.delete(frame.sub_id);
        return;
      }
      const method = frame.method;
      if (method && CHAT_METHODS[method]) {
        const result = await CHAT_METHODS[method](frame.params || {});
        port.postMessage({ id: frame.id, result });
        return;
      }
    } catch (err) {
      port.postMessage({ id: frame.id, error: { message: err?.message ?? String(err) } });
    }
  });
  port.onDisconnect.addListener(() => {
    for (const s of localSubs) chatRuntimeSubscribers.delete(s);
    localSubs.clear();
  });
});